...
public class FraudDetectionTask {
    ...
    // Roda a cada 5 minutos = 300 000 milsegundos
    @Scheduled(fixedDelay = 300000)
    @Transactional
    public void scanForSuspiciousActivity() {
        log.info("Iniciando varredura de fraude...");
        LocalDateTime fiveMinutesAgo = LocalDateTime.now().minusMinutes(5);
        BigDecimal highValueThreshold = new BigDecimal("200000.00");

        //Busca IDs de contas que tiveram mais de 3 transações > 3000000 nos últimos 5 min
